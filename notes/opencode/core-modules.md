...
## Context 管理 / Compaction 機制

### Context Window 決策機制

各家 model 的 context window 大小由 **`models.dev`** 動態拉取，每個 model 帶有：

| 欄位 | 說明 |
|------|------|
| `limit.context` | 模型總 context window（input + output 合計） |
| `limit.input` | 純 input 上限（部分模型有獨立限制，可選）|
| `limit.output` | 最大 output token 數 |

`ProviderTransform.maxOutputTokens(model)` 計算公式：
```
min(model.limit.output, OUTPUT_TOKEN_MAX) || OUTPUT_TOKEN_MAX
```
OUTPUT_TOKEN_MAX 預設 **32,000**，可透過環境變數 `OPENCODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX` 覆蓋。

**可用 input 空間計算**（`session/overflow.ts`）：
```
usable = limit.input
  ? limit.input - reserved           // 模型有獨立 input 限制
  : limit.context - maxOutputTokens  // 從 context 中扣除 output 空間
```

其中 `reserved` 緩衝 = `config.compaction.reserved ?? min(20_000, maxOutputTokens)`，確保永遠保留足夠空間給 LLM 輸出。

### Context Overflow 偵測（`session/overflow.ts`）

當已用 token（含 cache read/write）超過 `usable` 時，自動觸發 `session/compaction.ts` 壓縮：

- `limit.context === 0`（provider 未回報 context 大小）→ 跳過 compaction
- 可透過 `config.compaction.auto = false` 停用自動 compaction
- 可透過 `config.compaction.reserved` 自訂預留緩衝大小

**錯誤式 overflow 偵測**（`provider/error.ts`）：

token 計算不一定準確（尤其是 custom/local provider），因此 opencode 另外維護 overflow error pattern 列表，對各家 provider 的 context 超出錯誤訊息進行 regex 比對：
...
| Mistral / Cerebras | `400 / 413 (no body)` HTTP 狀態碼 |
| Ollama | `prompt too long; exceeded.*context length` |
| LM Studio | `greater than the context length` |
| llama.cpp | `exceeds the available context size` |
| vLLM | `context length is only \d+ tokens` |
| （通用）| `context_length_exceeded`、`context window exceeds limit` 等 |

一旦命中任一 pattern，視為 overflow 並觸發 compaction 或 retry 流程。
